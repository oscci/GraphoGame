...
rho       <- .57 # correlation between time 1 and time 2 scores, from Table 3 of NFER report
EffSize   <- -.06 # average treatment effect: from Table 6 of NFER report
nsims     <- 500 #number of simulations: set to a high number for final run
...
### Inquiry  
The inquiry specifies what parameters we want to estimate from the model - known as the estimand. This could be a descriptive statistic, such as the mean value of a variable, or the difference or correlation between variables. Here we specify the mean difference between intervention and control groups at time 2 as the estimand.
...
The answer strategy involves first fitting a statistical model to data, and then summarising the model fit. The example of the pretest_posttest function in the DeclareDesign vignette compares three different analytic approaches, all implemented in linear models. This is less relevant to our purposes, and so we will focus just on an analysis corresponding to that adopted in the NFER report;  predicting outcome from two covariates: treatment group (Z) and pretest score (Y_t1). (In fact, the NFER report added classroom as a further covariate, but for simplicity we will ignore that here).
...
designs <- redesign(NFER_design,EffSize=c(-.06,.17),rho=c(.6,.8))
...
designs <- redesign(Ahmed_design,r_L.u2=c(0,.2,.4),EffSize=c(-.06,.17))
...
save(Ahmed_diagnosis,file="Ahmed.RData")
save(NFER_diagnosis,file="NFER.RData")

# plot simulations

ggplot(Ahmed_sims,aes(estimate)) + 
    geom_histogram(data=subset(Ahmed_sims,sig == 1),fill = "red", alpha = 0.4) +
    geom_histogram(data=subset(Ahmed_sims,sig == 0),fill = "blue", alpha = 0.4) +
    geom_vline(aes(xintercept = estimand),
             lty = "dashed", color = "#C6227F") +
    facet_grid(r_L.u2~EffSize,labeller=label_both) +
  labs(x = "Estimate", y = "Count of simulations") +
  ggtitle("Ahmed method: rho = .6")


ggplot(NFER_sims,aes(estimate)) + 
    geom_histogram(data=subset(NFER_sims,sig == 1),fill = "red", alpha = 0.4) +
    geom_histogram(data=subset(NFER_sims,sig == 0),fill = "blue", alpha = 0.4) +
    geom_vline(aes(xintercept = estimand),
             lty = "dashed", color = "#C6227F") +
    facet_grid(rho~EffSize,labeller=label_both) +
  labs(x = "Estimate", y = "Count of simulations") +
  ggtitle("NFER method: rho = .6 or .8")
```
